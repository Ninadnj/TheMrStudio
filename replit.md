...
- **2025 Warm Minimalist Design System**: Complete rebrand with elegant color palette: rich black (#0B0B0B), off-white (#F5F5F5), neutral grey (#BDBDBD), and rose gold accent (#B76E79). Removed all duplicate CSS blocks to ensure single source of truth for colors. Hero section redesigned with bold "THE MR Studio" typography, warm dark background with blurred cover photo, and subtle grain/noise overlay. Footer enhanced with rose gold gradient line and smooth social icon hover fades. All themes updated to support both light and dark modes.
