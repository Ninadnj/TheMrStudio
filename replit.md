...
- **2025 Warm Minimalist Design System**: Complete rebrand with warm color palette: rich black (#0B0B0B), off-white (#F5F5F5), neutral grey (#BDBDBD), and caramel accent (#C6A58A). Removed all duplicate CSS blocks to ensure single source of truth for colors. Hero section redesigned with bold "Precision Beauty. Minimal effort." typography, subtle grain/noise overlay, and caramel accent line. Footer enhanced with warm caramel gradient line and smooth social icon hover fades. All themes updated to support both light and dark modes.
