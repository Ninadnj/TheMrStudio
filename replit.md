...
- **Gallery PostgreSQL Migration**: Migrated gallery images from in-memory storage to PostgreSQL for data persistence. Created gallery_images table and updated storage layer to use Drizzle ORM. Gallery now displays 12 sample images across 3 categories that persist across server restarts. All CRUD operations (create, update, delete) use database queries instead of in-memory Map.
